import kfp
from kfp import dsl
import kfp_preprocess_component
import kfp_train_component
import kfp_test_component
import kfp_deploy_model_component
...
def california_housing_pipeline():
    _preprocess_op = kfp_preprocess_component.preprocess_op()

    _train_op = kfp_train_component.train_op(
        dsl.InputArgumentPath(_preprocess_op.outputs['x_train']),
        dsl.InputArgumentPath(_preprocess_op.outputs['y_train'])
    ).after(_preprocess_op)

    _test_op = kfp_test_component.test_op(
        dsl.InputArgumentPath(_preprocess_op.outputs['x_test']),
        dsl.InputArgumentPath(_preprocess_op.outputs['y_test']),
        dsl.InputArgumentPath(_train_op.outputs['model'])
    ).after(_train_op)

    _deploy_model_op = kfp_deploy_model_component.deploy_model_op(
        dsl.InputArgumentPath(_train_op.outputs['model'])
    ).after(_test_op)
#
#
...
def call_kubeflow_pipeline():
    client = kfp.Client(host='http://host.docker.internal:8081')
    client.create_run_from_pipeline_func(california_housing_pipeline, arguments={})
